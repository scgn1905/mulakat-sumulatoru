...
import { Mail, User, Clock, MessageSquare, ShieldCheck, Users, PlusCircle, AlertTriangle, Trash2, BarChart3, Database, Award, Send, Activity } from 'lucide-react';
...
export default function Admin() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Tüm Mülakat Sonuçlarını (Liderlik Tablosu verisini) çek
  ...
  const handleDeleteUser = async (userId, userEmail) => {
    ...
    // Geçici olarak frontend listesinden düşürelim (Backend'e silme route'u eklenebilir)
    setUsersList(usersList.filter(u => u.id !== userId));
    alert("Kullanıcı başarıyla sistemden kaldırıldı.");
  };

  // Duyuru Gönderme Simülasyonu
  const handleSendAnnouncement = (e) => {
    e.preventDefault();
    if (!announcementText.trim()) return;
    alert(`Duyuru başarıyla tüm kullanıcılara iletildi:\n\n"${announcementText}"`);
    setAnnouncementText('');
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* 3. SEKME: KULLANICI LİSTESİ & ENGELLEME */}
            {activeTab === 'users' && (
            ...
            {/* 5. SEKME: TÜM MÜLAKAT SONUÇLARI İNCELEMESİ */}
            {activeTab === 'results' && (
              <div className="bg-slate-900/60 border border-slate-800 rounded-3xl overflow-hidden p-6 space-y-6">
                <h2 className="text-lg font-bold flex items-center gap-2">
            ...
            {/* 6. SEKME: DUYURU VE BİLDİRİM YÖNETİMİ */}
            {activeTab === 'announcement' && (
              <div className="bg-slate-900/60 border border-slate-800 p-8 rounded-3xl space-y-6 max-w-2xl mx-auto">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Tüm platform kullanıcılarına bildirim gönderin</p>
                  </div>
                </div>
